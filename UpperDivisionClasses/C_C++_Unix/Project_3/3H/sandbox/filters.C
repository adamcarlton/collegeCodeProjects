...
Shrinker::Execute()
{
    ...
    for(i = 0; i < halfWidth; i++){
        for(j = 0; j<halfHeight; j++){
            output->getBuffer()[j*halfWidth+i].red = img1->getBuffer()[2*j*ogWidth + 2*i].red;
            output->getBuffer()[j*halfWidth+i].green = img1->getBuffer()[2*j*ogWidth + 2*i].green;
            output->getBuffer()[j*halfWidth+i].blue = img1->getBuffer()[2*j*ogWidth + 2*i].blue;
        }
    }
};
...
Checkerboard::Execute(){
  ...
  if(img1->getWidth() != img2->getWidth()){
    char msg[1024];
    sprintf(msg, "%s: Widths do not match. Sizes must be identical", SinkName());
    DataFlowException e(SinkName(), msg);
    throw e;
  }
  if(img1->getHeight() != img2->getHeight()){
    char msg[1024];
    sprintf(msg, "%s: Heights do not match. Sizes must be identical", SinkName());
    DataFlowException e(SinkName(), msg);
    throw e;
  }
  int width = img1->getWidth();
